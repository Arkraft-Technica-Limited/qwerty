...
import tech.arkraft.qwerty.nostr.NostrTransport
...
class MessageRouter private constructor(
            ...
            val instance = INSTANCE ?: synchronized(this) {
                INSTANCE ?: run {
                    ...
                    MessageRouter(context.applicationContext, mesh, nostr).also { instance ->
                        ...
                        try {
                            tech.arkraft.qwerty.favorites.FavoritesPersistenceService.shared.addListener(instance.favoriteListener)
                        } catch (_: Exception) {}
                        INSTANCE = instance
                    }
                }
    ...
    private val favoriteListener = object: tech.arkraft.qwerty.favorites.FavoritesChangeListener {
    ...
    fun sendPrivate(content: String, toPeerID: String, recipientNickname: String, messageID: String) {
        ...
        if (tech.arkraft.qwerty.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            Log.d(TAG, "Routing PM via Nostr (geohash) to alias ${toPeerID.take(12)}… id=${messageID.take(8)}…")
            val recipientHex = tech.arkraft.qwerty.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                // Resolve the conversation's source geohash, so we can send from anywhere
                val sourceGeohash = tech.arkraft.qwerty.nostr.GeohashConversationRegistry.get(toPeerID)
    ...
    fun sendDeliveryAck(messageID: String, toPeerID: String) {
        ...
        if (tech.arkraft.qwerty.nostr.GeohashAliasRegistry.contains(toPeerID)) {
            val recipientHex = tech.arkraft.qwerty.nostr.GeohashAliasRegistry.get(toPeerID)
            if (recipientHex != null) {
                nostr.sendDeliveryAckGeohash(messageID, recipientHex, try { tech.arkraft.qwerty.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)!! } catch (_: Exception) { return })
                return
            }
        }
        if (!((mesh.getPeerInfo(toPeerID)?.isConnected == true) && mesh.hasEstablishedSession(toPeerID))) {
    ...
    fun sendFavoriteNotification(toPeerID: String, isFavorite: Boolean) {
        if (mesh.getPeerInfo(toPeerID)?.isConnected == true) {
            val myNpub = try { tech.arkraft.qwerty.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub } catch (_: Exception) { null }
    ...
    private fun canSendViaNostr(peerID: String): Boolean {
        return try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                val fav = tech.arkraft.qwerty.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val fav = tech.arkraft.qwerty.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
                fav?.isMutual == true && fav.peerNostrPublicKey != null
            } else {
                false
            }
